...
for i in naver_urls:
    ...
    # 기자 이름 가져오기 #writer-profile > div.right > div > p.name
    name = html.select("#wrap > header > div.header-inner > strong > a > span")
    print(name)
    # list합치기
    name = ''.join(str(name))
    # html태그제거
    pattern1 = '<[^>]*>'
    name = re.sub(pattern=pattern1, repl='', string=title)
    names.append(name)
